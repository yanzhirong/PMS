...
namespace Dal
{
    public class DalMaterialsOut
    {
        ...
        public DataTable GetMaterialsOut(String _materialsName, int _applyMemberId, int _factoryId, DateTime _beginTime, DateTime _endTime, int _outputStatus)
        {
            ...
            sbSql.Append("       a1.requestOutputNum numDisplay, ");
            ...
            sbSql.Append("join r_materials_output_detail a1 ");
            sbSql.Append("  on a.outputCode = a1.outputCode ");
            sbSql.Append("left join m_factory b ");
            sbSql.Append("  on a.factoryId = b.id ");
            sbSql.Append("left join p_materials c ");
            sbSql.Append("  on a1.materialsId = c.id ");
            ...
            {
                sbSql.Append("   and a1.materialsId in (");
        ...
        {
            sbSql.Clear();
            sbSql.Append("select a.outputNum, ");
        ...
        public DataTable GetMaterialsOutByProduceCode(string _produceCode)
        {
            sbSql.Clear();
            sbSql.Append("select a.* ");
            sbSql.Append("from p_materials_output a ");
        ...
        public int UpdateMaterialsOut(ModelMaterialsOutput _model)
        {
            ...
            foreach (ModelMaterialsOutputDetail modelMaterialsOutputDetail in _model.modelMaterialsOutputDetail)
            {
                sbSql.Clear();
                sbSql.Append("update r_materials_output_detail ");
                sbSql.Append("set requestOutputNum = '" + modelMaterialsOutputDetail.requestOutputNum + "',");
                sbSql.Append("    specsId = " + modelMaterialsOutputDetail.specsId + ",");
                sbSql.Append("    modifyBy = '" + modelMaterialsOutputDetail.modifyBy + "',");
                sbSql.Append("    modifyTime = '" + modelMaterialsOutputDetail.modifyTime + "' ");
                sbSql.Append("where id = " + modelMaterialsOutputDetail.id);
        ...
        public DataTable GetMaterialsOutSelect(int _factoryId, int _materialsId, int _specsId)
        {
            ...
            sbSql.Append("       a.specsId, ");
            sbSql.Append("       ifnull(c.name, '件') specsName, ");
            sbSql.Append("       a.stockNum num, ");
            sbSql.Append("       case when ifnull(a.isQualified, 0) > 0 then '有' else '无' end QC, ");
            ...
            sbSql.Append("left join r_materials_specs c ");
            sbSql.Append("  on a.materialsId = c.materialsId ");
            sbSql.Append(" and a.specsId = c.id ");
            sbSql.Append("where a.isDelete = 0 ");
            sbSql.Append("  and a.factoryId = ").Append(_factoryId).Append(" ");
        ...
        public DataTable GetMaterialsOutDetailByOutputCode(string _outputCode)
        {
            ...
            sbSql.Append("select a.id, ");
            sbSql.Append("       b.name materialsName, ");
            sbSql.Append("       a.requestOutputWeight , ");
            sbSql.Append("       a.specsId, ");
            sbSql.Append("       a.requestOutputNum , ");
            sbSql.Append("       a.realityOutputNum , ");
            sbSql.Append("       case a.outputStatus when 0 then '请求出库' else '完成出库' end statusName, ");
            sbSql.Append("       a.materialsId, ");
            sbSql.Append("       a.outputStatus statusCode ");
            sbSql.Append("from r_materials_output_detail a ");
            sbSql.Append("join p_materials b ");
            sbSql.Append("  on a.materialsId = b.id ");
            sbSql.Append("where a.isDelete = 0 ");
            sbSql.Append("  and a.outputCode = '").Append(_outputCode).Append("' ");
        ...
        public DataTable GetMaterialsOutDetailById(int _outputDetailId)
        {
            sbSql.Clear();
            sbSql.Append("select a.* ");
            sbSql.Append("from r_materials_output_detail a ");
            sbSql.Append("where a.isDelete = 0 ");
            sbSql.Append("  and a.id = ").Append(_outputDetailId).Append("");

            return Dal.DBHelper.Select(sbSql.ToString());
        }

        public int AddMaterialsOutputDetail(ModelMaterialsOutputDetail _model)
        {
            sbSql.Clear();
            sbSql.Append("insert into ");
            ...
            sbSql.Append("       " + _model.requestOutputNum + ", ");
            sbSql.Append("       " + _model.outputStatus + ", ");
            sbSql.Append("       " + _model.isDelete + ", ");
            sbSql.Append("      '" + _model.createBy + "', ");
            sbSql.Append("      '" + _model.createTime + "')");
        ...
        public int DeteleMaterialsOutputDetail(ModelMaterialsOutputDetail _model)
        {
            sbSql.Clear();
            sbSql.Append("update r_materials_output_detail ");
            sbSql.Append("set isDelete = 1,");
        ...
        public int doOutPut(string _outputCode, int _outputDetailId, int _factoryId, int _materialsId, decimal _outputNum, int _applyMemberId, List<Dictionary<string, object>> listOutput, string userName)
        {
            List<string> listSql = new List<string>();
            ...
            foreach (Dictionary<string, object> dc in listOutput)
            {
                ...
                sbSql.Append("       " + ConvertUtils.ConvertToDecimal(dc["specsId"]) + ", ");
            ...
            sbSql.Append(" where id = ").Append(_outputDetailId).Append(" ");
            sbSql.Append("   and isDelete = 0 ");
            sbSql.Append("   and (realityOutputNum >= requestOutputNum or ifnull(requestOutputNum, 0) <= 0) ");
            listSql.Add(sbSql.ToString());
