...
namespace Dal
{
    public class DalSaleOrder
    {
        ...
        public DataTable GetSaleOrders(String _code, String _name, int _salerId, int _status, DateTime _beginTime, DateTime _endTime, int _roleType)
        {
            ...
            sbSql.Append("       case a.orderStatus when ").Append((int)Enum.EnumSaleOrderStatus.ProducingApply).Append(" then '编辑' ");
            sbSql.Append("                          when ").Append((int)Enum.EnumSaleOrderStatus.Transporting).Append(" then '编辑' else '查看' end modifyBtn, ");
            sbSql.Append("       '取消' cancelBtn ");
        ...
        public DataTable GetSaleOrderDetailByOrderCode(string _orderCode)
        {
            sql = @"select a.id,
                           ...
                           d.advisePrice,
                           '删除' deleteBtn
                      from r_saleOrder_Detail a
                      left join p_product b
                        on a.productId = b.id
                      left join r_product_specs c
                        on a.productId = c.productId
                       and a.specsId = c.id
                       and c.isDelete = 0
                      left join r_product_price d
                        on a.productId = d.productId
                       and a.specsId = d.specsId
                       and d.isDelete = 0
                     where a.isDelete = 0
                       and a.orderCode = '{0}'";
        ...
        public DataTable GetSaleOrderDetailByProductSpecs(string _orderCode, int _productId, int _specsId)
        {
            sql = @"select a.*
                      from r_saleOrder_Detail a
                     where a.isDelete = 0
            ...
            sql = String.Format(sql, _orderCode, _productId, _specsId);

            return Dal.DBHelper.Select(sql);
        }

        public DataTable GetSaleOrderDetailByProductSpecs(int _productId, int _specsId)
        {
            sql = @"select a.*
                      from r_saleOrder_Detail a
                     where a.isDelete = 0
        ...
        public int AddSaleOrder(ModelSaleOrder _model)
        {
            ...
            sbSql.Append("       " + _model.transPrice + ", ");
            ...
            //    sbSql.Append("       " + modelDetail.specsId + ", ");
        ...
        public int UpdateSaleOrder(ModelSaleOrder _model)
        {
            ...
            sbSql.Append("    transPrice = " + _model.transPrice + ",");
            ...
            foreach (ModelSaleOrderDetail modelDetail in _model.modelSaleOrderDetail)
            {
                sbSql.Clear();
                sbSql.Append("update r_saleOrder_Detail ");
                sbSql.Append("set num = " + modelDetail.num + ",");
                sbSql.Append("    price = " + modelDetail.price + ",");
                sbSql.Append("    remark = '" + modelDetail.remark + "',");
                sbSql.Append("    modifyBy = '" + modelDetail.modifyBy + "',");
                sbSql.Append("    modifyTime = '" + modelDetail.modifyTime + "' ");
                sbSql.Append("where id = " + modelDetail.id);
                listSql.Add(sbSql.ToString());
            }
            return Dal.DBHelper.ExcuteTransaction(listSql);
        }
        ...
        public int ConfirmSaleOrder(ModelSaleOrder _modelSaleOrder, ModelProductOutput _modelProductOutput, ModelFinanceReceive _modelFinanceReceive)
        {
            ...
            sbSql.Clear();
            sbSql.Append("delete from r_product_output_detail ");
            sbSql.Append(" where outputCode in (select outputCode from p_product_output where orderCode = '" + _modelProductOutput.orderCode + "' and isDelete = 0 ) ");
            sbSql.Append("   and isDelete = 0 ");
            listSql.Add(sbSql.ToString());

            sbSql.Clear();
            sbSql.Append("delete from p_product_output ");
            sbSql.Append(" where orderCode = '" + _modelProductOutput.orderCode + "' ");
            sbSql.Append("   and isDelete = 0 ");
            listSql.Add(sbSql.ToString());
            ...
            foreach (ModelProductOutputDetail modelProductOutputDetail in _modelProductOutput.modelProductOutputDetail)
            {
                ...
                sbSql.Append("       " + modelProductOutputDetail.specsId + ", ");
            ...
            sbSql.Clear();
            sbSql.Append("delete from p_finance_receive ");
            sbSql.Append(" where orderCode = '" + _modelFinanceReceive.orderCode + "' ");
            sbSql.Append("   and isDelete = 0 ");
            listSql.Add(sbSql.ToString());
            ...
            sbSql.Clear();
            sbSql.Append("delete from p_finance_accounts ");
            sbSql.Append("where orderCode = '" + _modelSaleOrder.orderCode + "'");
            listSql.Add(sbSql.ToString());
        ...
        public int DeleteSaleOrder(ModelSaleOrder _model)
        {
            ...
            sbSql.Append("    modifyBy = '" + _model.modifyBy + "',");
            sbSql.Append("    modifyTime = '" + _model.modifyTime + "' ");
            sbSql.Append(" where outputCode in (select outputCode from p_product_output where orderCode = '" + _model.orderCode + "' and isDelete = 0 ) ");
            sbSql.Append("   and isDelete = 0 ");
            listSql.Add(sbSql.ToString());
        ...
        public int AddSaleOrderDetail(ModelSaleOrderDetail _model)
        {
            sbSql.Clear();
            sbSql.Append("insert into ");
            sbSql.Append("       r_saleOrder_Detail ( ");
        ...
        public int DeteleSaleOrderDetail(ModelSaleOrderDetail _model)
        {
            sbSql.Clear();
            sbSql.Append("update r_saleOrder_Detail ");
            sbSql.Append("set isDelete = 1,");
