    ...
    public class BllProductOut : Bll.BllBase
    {
        ...
        public Boolean CheckUpdateDelete(string _outputCode)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    if(ConvertUtils.ConvertToInt(dr["statusCode"]) > 0)
                    {
                        return false;
                    }
                }
        ...
        {
            DataTable dt = m_dalProductOut.GetProductOutSelect(_factoryId, _productId, _specsId);

            return dt;

        }

        public Boolean AddProductOutDetail(ModelProductOutputDetail _model)
        {
            int rtn = 0;
            rtn = m_dalProductOut.AddProductOutDetail(_model);

            return rtn == 0 ? false : true;
        }

        public Boolean DeleteProductOutDetail(ModelProductOutputDetail _model)
        {
            int rtn = 0;
            DataTable dt = m_dalStore.GetProductOutputLogByOutputCode(_model.outputCode);
            Dictionary<string, int> dc = new Dictionary<string, int>();
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string inputCode = ConvertUtils.ConvertToString(dr["inputCode"]);
                    if (dc.ContainsKey(inputCode))
                    {
                        int outputNum = dc[inputCode];
                        dc[inputCode] = outputNum + ConvertUtils.ConvertToInt(dr["outputNum"]);
                    }
                    else
                    {
                        dc.Add(inputCode, ConvertUtils.ConvertToInt(dr["outputNum"]));
                    }
                }
            }
            rtn = m_dalProductOut.DeleteProductOutDetail(_model, dc);

            return rtn == 0 ? false : true;
        }

        public Boolean doOutPut(string _orderCode, string _outputCode, int _outputDetailId, int _factoryId, int _productId, int _specsId, int _applyMemberId, int _outputNum, List<Dictionary<string, object>> listOutput, string userName)
        {
            int rtn = m_dalProductOut.doOutPut(_orderCode, _outputCode, _outputDetailId, _factoryId, _productId, _specsId, _applyMemberId, _outputNum, listOutput, userName);
