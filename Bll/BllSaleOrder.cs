...
namespace Bll
{
    public class BllSaleOrder : Bll.BllBase
    {
        ...
        public ModelSaleOrderDetail GetSaleOrderDetailByProductSpecs(string _orderCode, int _productId, int _specsId)
        {
            ModelSaleOrderDetail model = new ModelSaleOrderDetail();

            DataTable dt = m_dalSaleOrder.GetSaleOrderDetailByProductSpecs(_orderCode, _productId, _specsId);

            if (dt != null && dt.Rows.Count > 0)
            {
                model = ModelUtils<ModelSaleOrderDetail>.FillModel(dt.Rows[0]);
        ...
        public DataTable GetSaleOrderDetailByProductSpecs(int _productId, int _specsId)
        {
            ...
            DataTable dt = m_dalSaleOrder.GetSaleOrderDetailByProductSpecs(_productId, _specsId);

            return dt;
        }
        public Boolean AddSaleOrder(ModelSaleOrder _model, bool _isConfirm)
        {
            int rtn = 0;
            string orderCode = BllSeq.GetCode("xxdd", "saleCode");
            _model.orderCode = orderCode;
            _model.orderStatus = (int)Enum.EnumSaleOrderStatus.Transporting;
            rtn = m_dalSaleOrder.AddSaleOrder(_model);

            if (rtn > 0)
            {
                _model.id = ConvertUtils.ConvertToInt(m_dalSaleOrder.GetSaleOrderByOrderCode(_model.orderCode).Rows[0]["id"]);
                return ConfirmSaleOrder(_model);
                //if (_isConfirm)
                //{
                //    return ConfirmSaleOrder(_model);
                //}
        ...
        public Boolean UpdateSaleOrder(ModelSaleOrder _model, bool _isConfirm)
        {
            int rtn = 0;
        ...
        public Boolean ConfirmSaleOrder(ModelSaleOrder _modelSaleOrder)
        {
            ...
            //等待出库
            _modelSaleOrder.orderStatus = (int)Enum.EnumSaleOrderStatus.Transporting;

            //出库申请单
            ModelProductOutput modelProductOutput = new ModelProductOutput();
            string outputCode = BllSeq.GetCode("cpck", "productOutCode");
            ...
            foreach (ModelSaleOrderDetail modelSaleOrderDetail in _modelSaleOrder.modelSaleOrderDetail)
            {
                ...
                modelProductStoreOutDetail.specsId = modelSaleOrderDetail.specsId;
            ...
            modelFinanceReceive.orderPrice = _modelSaleOrder.price + _modelSaleOrder.transPrice;
            modelFinanceReceive.salerId = _modelSaleOrder.salerId;
            modelFinanceReceive.financeId = 0;
        ...
        public string CheckStore(ModelSaleOrder _model)
        {
            ...
            foreach (ModelSaleOrderDetail saleProduct in listSaleProduct)
            {
                ...
                //查询成品库存
                int storeNum = m_bllStore.GetStoreProductNum(_model.factoryId, saleProduct.productId, saleProduct.specsId, _model.deliveryDate);
        ...
        public Boolean AddProduceApply(ModelSaleOrder _model)
        {
            ...
            foreach (ModelSaleOrderDetail saleProduct in _model.modelSaleOrderDetail)
            {
                ...
                if (saleProduct.num > storeNum)
                {
                    ...
                    modelProduceApply.specsId = saleProduct.specsId;
        ...
        public Boolean AddSaleOrderDetail(ModelSaleOrderDetail _model)
        {
            int rtn = 0;
            rtn = m_dalSaleOrder.AddSaleOrderDetail(_model);

            return rtn == 0 ? false : true;
        }

        public Boolean DeteleSaleOrderDetail(ModelSaleOrderDetail _model)
        {
            int rtn = 0;
            rtn = m_dalSaleOrder.DeteleSaleOrderDetail(_model);
