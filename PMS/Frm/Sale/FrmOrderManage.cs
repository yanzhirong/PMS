...
{
    public partial class FrmOrderManage : Main.BaseForm
    {
        //销售ID
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.Columns[e.ColumnIndex].Name == "modifyBtn")
            {
                ...
                if (orderStatus == (int)Enum.EnumSaleOrderStatus.ProducingApply || orderStatus == (int)Enum.EnumSaleOrderStatus.Transporting)
                {
            ...
            if (dataGridView1.Columns[e.ColumnIndex].Name == "cancelBtn")
            {
                ...
                {
                    int id = (int)dataGridView1.Rows[e.RowIndex].Cells["id"].Value;
                    Form form = new FrmOrderDetail(5, id);
                    form.ShowDialog();
                    doSelect();
                }
                if (orderStatus == (int)Enum.EnumSaleOrderStatus.Cancel)
                {
                    Common.Tools.MsgUtils.ShowInfoMsg("订单已取消！");
                    return;
                }
                if (orderStatus == (int)Enum.EnumSaleOrderStatus.Complete)
                {
                    Common.Tools.MsgUtils.ShowErrorMsg("订单已完成，不可取消！");
                    return;
                }
                if (orderStatus == (int)Enum.EnumSaleOrderStatus.Producing)
                {
                    if (Common.Tools.MsgUtils.ShowQustMsg("订单已开始生产，确认取消？", MessageBoxDefaultButton.Button2) == System.Windows.Forms.DialogResult.Yes)
                    {
                        int id = (int)dataGridView1.Rows[e.RowIndex].Cells["id"].Value;
                        Form form = new FrmOrderDetail(5, id);
                        form.ShowDialog();
