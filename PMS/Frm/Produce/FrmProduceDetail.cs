...
namespace PMS.Frm.Produce
{
    public partial class FrmProduceDetail : Main.BaseForm
    {
        ...
        private Boolean doSubmit(bool _isConfirm)
        {
            ...
            if (m_mode == 0) 
            {
                string produceCode = m_bllProduce.AddProduce(model);
                ...
                else
                {
                    ...
                    m_produceId = modelProduce.id;
                    this.txt_produceCode.Text = modelProduce.produceCode;
                    m_mode = 1;
                    if (_isConfirm == true)
                    {
        ...
        private void btn_submitMaterials_Click(object sender, EventArgs e)
        {
            if (m_mode == 0 || m_mode == 1)
            {
        ...
        private void btn_materialsOutput_Click(object sender, EventArgs e)
        {
            if(m_mode == 0 || m_mode == 1)
            {
                if(doSubmit(false) == false)
            ...
            ModelMaterialsOutput modelMaterialsOutput = m_bllMaterialsOut.GetMaterialsOutByProduceCode(produceCode);

            if (modelMaterialsOutput.id > 0)
            {
                Form form = new Store.FrmMaterialsOutDetail(3, 1);
                form.ShowDialog();
            }
        }
    }
