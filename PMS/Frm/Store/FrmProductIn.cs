...
{
    public partial class FrmProductIn : Main.BaseForm
    {
        ...
        private void doSelect()
        {
            ...
            int isQC = -1;
            if (this.cmb_QC.SelectedIndex > 0)
            {
                isQC = this.cmb_QC.SelectedIndex - 1;
            }

            DataTable dt = m_bllProductIn.GetProductIn(productName, factoryId, beginTime, endTime, inputStatus, isQC);
        ...
        {
            //质检
            if (dataGridView1.Columns[e.ColumnIndex].Name == "QC")
            {

                string inputCode = (string)dataGridView1.Rows[e.RowIndex].Cells["inputCode"].Value;
                int outputCnt = ConvertUtils.ConvertToInt(dataGridView1.Rows[e.RowIndex].Cells["outputCnt"].Value);
                Form form;

                if (outputCnt == 0)
                {
                    form = new FrmProductInQC(1, inputCode);
                }
                else
                {
                    form = new FrmProductInQC(3, inputCode);
                }
                form.ShowDialog();
                doSelect();
            ...
            if (dataGridView1.Columns[e.ColumnIndex].Name == "deleteBtn")
            {
                ...
                if (outputCnt > 0)
                {
                    Common.Tools.MsgUtils.ShowInfoMsg("此成品已出库，不可删除！");
                    return;
                }
