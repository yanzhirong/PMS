...
namespace PMS.Frm.Store
{
    public partial class FrmMaterialsIn : Main.BaseForm
    {
        ...
        private void doSelect()
        {
            ...
            DataTable dt = m_bllMaterialsIn.GetMaterialsIn(materialsName, customerId, factoryId, beginTime, endTime, inputStatus, isQC);
        ...
        {
            //质检
            if (dataGridView1.Columns[e.ColumnIndex].Name == "isQualified")
            {
                ...
                if (outputCnt == 0)
                {
                    form = new FrmMaterialsInQC(1, inputCode);
                }
                else
                {
                    form = new FrmMaterialsInQC(3, inputCode);
                }
                form.ShowDialog();
                doSelect();
            ...
            if (dataGridView1.Columns[e.ColumnIndex].Name == "deleteBtn")
            {
                ...
                if (outputCnt > 0)
                {
                    Common.Tools.MsgUtils.ShowInfoMsg("此原料已使用，不可删除！");
                    return;
                }
