...
{
    public partial class FrmMaterialsInDetail : Main.BaseForm
    {
        ...
        {      
            ...
            if (m_mode != 0 && m_materialsInId > 0)
            {
                ...
                //原料类型
                this.cmb_type.SelectedIndex = m_bllMaterials.GetMaterialsById(model.materialsId).type;
        ...
        private Boolean doSubmit(bool showMsg)
        {
            ...
            modelMaterialsIn.purchaseNum = ConvertUtils.ConvertToInt(this.txt_purchaseNum.Text.Trim());
            ...
            {
                modelMaterialsIn.specsId = ConvertUtils.ConvertToInt(((ModelItem)this.cmb_specs.SelectedItem).itemKey);
            }
            ...
            modelMaterialsIn.inputNum = ConvertUtils.ConvertToInt(this.txt_inputNum.Text.Trim());
            modelMaterialsIn.stockNum = ConvertUtils.ConvertToInt(this.txt_inputNum.Text.Trim());
        ...
        private Boolean doCheck()
        {
            if(StringUtils.IsNotBlank(m_inputCode))
            {
                ...
                if (m_mode == 1)
                {
                    ...
                    {
                        MsgUtils.ShowErrorMsg("已有部分原料出库，不可修改！");
                        return false;
                    }
                }
                ...
                if (m_mode == 2)
                {
                    if (m_bllMaterialsIn.CheckUpdateDelete(m_inputCode) == false)
                    {
                        MsgUtils.ShowErrorMsg("已有部分原料出库，不可删除！");
                        return false;
                    }
                }
            }
        ...
        private void btn_qc_Click(object sender, EventArgs e)
        {
            ...
            Form form = new FrmMaterialsInQC(m_mode,m_inputCode);
            form.ShowDialog();
        }
        ...
        private void btn_queryPurchase_Click(object sender, EventArgs e)
        {
            string purchaseCode = this.txt_purchaseCode.Text.Trim();
            if (StringUtils.IsNotBlank(purchaseCode))
            {
                Form form = new Purchase.FrmPurchaseDetail(3, m_bllPurchase.GetPurchaseByPurchaseCode(purchaseCode).id);
                form.ShowDialog();
            }
        }
