...
namespace PMS.Frm.Finance
{
    public partial class FrmReceivedDetail : Form
    {
        ...
        public FrmReceivedDetail(string _outputCode, int _outputDetailId, int _productId, int _factoryId, int _applyMemberId)
        {
            InitializeComponent();
        ...
        {
            //出库单号
            this.txt_orderCode.Text = m_outputCode;
            //工厂
        ...
        private void btn_applyProduce_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MsgUtils.ShowQustMsg("产品【" + this.txt_orderStatus.Text + "】已经申请生产，是否再次申请？", MessageBoxDefaultButton.Button2) == DialogResult.No)
                {
                    return;
                }
            }
            ...
            {
                if (MsgUtils.ShowQustMsg("是否确认申请生产产品【" + this.txt_orderStatus.Text + "】？", MessageBoxDefaultButton.Button1) == DialogResult.No)
                {
                    return;
                }
            }
